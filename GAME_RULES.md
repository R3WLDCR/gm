...
- ハンターは市民陣営。死亡時に生存者1名を必ず道連れにして発砲（道連れ死亡）する。自身の死亡によって勝敗が決まる場合は能力を行使できない。
- 狂人ハンターは人狼陣営。ハンターと同様に死亡時に生存者1名を必ず道連れにして発砲する。自身の死亡によって勝敗が決まる場合は能力を行使できない。
- てるてるは第3陣営。死亡（追放、襲撃、道連れ）した時点でてるてる陣営の勝利となる。複数人いる場合は全員が死亡した時点で勝利となる。複数指定できる。
- 占い・霊媒結果は、人狼だけが「人狼」、それ以外（ハンター、狂人ハンター、てるてる、裏切り者を含む）は「市民」となる。
...
- 通常投票の投票先は、直前の投票者が投票した対象が次の投票先候補にも含まれていれば初期選択として維持し、候補に含まれない場合や初回は候補の先頭を選択する。
- 投票中の最多候補には票数だけを表示する。
...
- ランダムボタンを押した後に1〜9の数字ボタンを押すと、1からその数字（分）までの範囲でランダムな分数をセットする。
- ランダムボタンを再度押すか、タイマーリセット・モード切り替えを行った場合はランダム待機を解除する。
...
- 進行ログの1日は朝の襲撃結果から始まり、昼・投票・追放を経て、その日の夜行動完了までとする。
- 各日のログは、朝から夜行動完了までを上から時系列順に表示する。
- 追放後の霊媒・護衛・占い・人狼の行動は、追放と同じ日のログへ含める。
- 翌日のログは、次の朝の襲撃結果から開始する。
...
- ただしログや「戻る」から襲撃結果を復元した場合は自動遷移を停止し、OKで確定済みの勝利画面へ進める。
